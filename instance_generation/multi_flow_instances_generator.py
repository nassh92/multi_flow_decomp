...
def update_time (transport_times, og_transport_times, capacities, og_capacities, arc):
...
def generate_multi_flow_instance(adj_mat, 
                                 capacities, 
                                 transport_times, 
    ...
    og_capacities = capacities.copy()
    og_transport_times = transport_times.copy()
    ...
    multi_flow = [[[0 for j in range(len(adj_mat))] for i in range(len(adj_mat))] for _ in range(len(pairs))]

    # If 'return_transition_function' is True, construct the transition functions
    if return_transition_function:
        trans_func, trans_from_sources, trans_to_destinations = construct_transition_functions(adj_mat, 
                                                                                               pairs)
